...
//add_action( 'mailster','add_lead_to_mailster' );
function writeDebugLog($errMsg){
    $debug=true;
   if ( $debug == true)  {
       BugFu::log($errMsg);
   } 
...
function add_lead_to_influencerSoft($post_email){
    $add_to_list=$_POST['influencersoftListID'];
     writeDebugLog('Adding User with email address =' . $post_email . "to Influencer Soft");
    $curl = curl_init(); curl_setopt_array($curl, array( 
    CURLOPT_URL => 'https://mindsetbliss.influencerSoft.com/api/addupdatelead',
      ...
      CURLOPT_POSTFIELDS => array('rpsKey' => '7c3b5db979716ef44adf89793d261549','lead_email'
     => $post_email,'add_to_lists' => $add_to_list,
     'remove_from_lists' => '','add_tags' => 'Online4Success Lead'),
   ...
   #   CURLOPT_CUSTOMREQUEST => 'POST',
    #  CURLOPT_POSTFIELDS => array('rpsKey' => #'7c3b5db979716ef44adf89793d261549','lead_email'
   #  => $post_email,'add_to_lists' => '1632350157.7223701213'#,'remove_from_lists' => '','add_tags' => 'Online4Success Lead'),
   #   
   # ));

    $response = curl_exec($curl);
  writeDebugLog('Response from Influencersoft :' . $response);
    curl_close($curl);
}

function add_lead_to_mailster(){
                $overwrite = true;
...
if( $duplead != 'yes' ) {
		...
		$name = $fullName;
		add_lead_to_influencerSoft($post_email);
		add_lead_to_mailster();
...
function wplp_get_referrer_id(){
	global $wp, $wpdb, $wplp_admin, $_GET, $_POST;
	$tmp=$_POST['_wp_http_referer'];
    $tmp=explode("=",$tmp);
    

    $ref_user=$tmp[1];
    writeDebugLog("**** ref_user=" . $ref_user);
	
	writeDebugLog("**** ref_user=" . $ref_user);
	writeDebugLog("**** COOKIE['wplp-ref']=" . $_COOKIE['wplp-ref']);
		$refCookie = $_COOKIE['wplp-ref'];
	if ( isset($ref_user) &&	$refCookie==null ){
	    writeDebugLog("Returning referring user" );
	    $field = 'login'; 
	    $user =get_user_by('login',$ref_user); // Get user obj
		if( $user) {
		writeDebugLog("Using refferal user id.");
			$ref_user_id = $user->ID;
		}
		writeDebugLog("ref_user_exists=" . $ref_user_exists);
		writeDebugLog("returning ref_user_id=" . $ref_user_id);
		return $ref_user_id;
	} else if( isset( $_COOKIE['wplp-ref'] ) && !empty( $_COOKIE['wplp-ref'] ) ) {
